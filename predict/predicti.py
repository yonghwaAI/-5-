...
class InputBuilder_BaselineModel:
  @staticmethod
  def history_to_dataframe(universe : dict):
    df = pd.DataFrame.from_records([{'dt':item.dt,'open':item.open, 'high':item.high, 'low':item.low, 'close':item.close, 'volume':item.volume} for item in universe.items])
    df['dt'] = pd.to_datetime(df['dt'], format='%Y%m%d%H%M').dt.tz_localize('Asia/Seoul')
    print(df)
    return df.set_index('dt')

from strategy.Strategy import *
IB = InputBuilder_BaselineModel()
strategy = Strategy()

IB.history_to_dataframe(strategy.universe)
